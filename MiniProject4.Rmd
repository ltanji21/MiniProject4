...
movie_info <- db %>%
  dbGetQuery("SELECT movie_id, info, note
             FROM imdb.movie_info 
             WHERE info_type_id = 1;")

#merging info about run times.
actual_movies <- movie_info %>%
  inner_join(movie_directors, by = "movie_id")
...
```{r}
all_directors_info <- actual_movies %>%
  inner_join(director_info, by = "person_id") %>%
  select(production_year, title, person_id, gender, movie_id, info, note) %>%
...
#Joining the number of directors total table to number of female directors table. 
...
female_director_graph <- ggplot(female_director_ratio, aes(x = production_year, y = ratio_f_directors)) + 
  ...
  geom_point(x = 1990, y = 13.4910751, colour = "black", size = 1) +
  ...
  geom_point(x = 2008, y = 16.2118126, colour = "red", size = 1)
