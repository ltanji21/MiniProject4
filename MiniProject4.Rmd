...
female_director_graph_best_film <- ggplot(female_director_ratio, aes(x = production_year, y = ratio_f_directors)) + 
  ...
  geom_line() + 
  labs(caption = "Percentages of female directed films by year. The red, yellow, and green points represent years where female directed films were nominated for Oscar's Best Picture award. 
Furthermore, the colors represent whether the nominated film passed the Bechdel test; green stands for passed, yellow represents uncategorized, and red means failed.") +
...
bechdel_graph_best_film <- ggplot(bechdel_all_films, aes(x = year, y = percent_passing)) + geom_point(alpha = .3) + ylab("% of films that passed the Bechdel test") + 
  xlab("Award Year") + 
  ggtitle("Nominated for Best Film and Passing the Bechdel Test") +
  geom_line() + scale_y_continuous(limits = c(0, 1)) + scale_x_continuous(limits = c(1990, 2016)) + theme(plot.title=element_text(family="Times", face="bold", size=16))



bechdel_graph_best_film
```
